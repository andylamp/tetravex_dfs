...
# classc DFS algorithm in order to find the first (not necessarety optimal) solution to the problem.
#
	...
	for tile in tiles:
		...
		if tile.using == False:
			...
			if checkValid(index,  tile):
					...
					if r != 'qq':
						return r
						tiles.sort(lambda x,y: (x.values["right"] == y.values["left"]) and (y.using==False));
...
def main():
	...
	#(and consiquently graph depth), the other is that we exausted our search space
